...
The bindgen-backed display and input crates select Homebrew's `libclang`
automatically when running `cargo test` on macOS; if LLVM is absent, the build
installs the `llvm` formula idempotently before generating bindings.
